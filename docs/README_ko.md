...
12개 전문 에이전트를 구조화된 파이프라인으로 오케스트레이션하여 소프트웨어 기능의 계획, 구현, 검증을 자동화한다.
...
UAM은 **Unified Agent Methodology**의 약자로, 에이전트 오케스트레이션을 위한 **독립형 플러그인**이다.

### 독립형 설계

UAM은 외부 프레임워크 없이 완전히 독립적으로 동작한다:
...
| 스킬 | 호출 | 목적 |
|------|------|------|
| **uam** | `/uam:uam` | 5-Phase 파이프라인 전체 실행 |
| **uam-small** | `/uam:uam-small` | 3-Phase 경량 파이프라인 (일상적 기능 추가) |
| **uam-pivot** | `/uam:uam-pivot` | Pivot Points 인터뷰 (불변 제약 정의) |
| **uam-status** | `/uam:uam-status` | 파이프라인 대시보드 (Phase, TODO, Gate, 수렴) |
| **uam-cancel** | `/uam:uam-cancel` | 안전한 중단 + 상태 보존 (`--force`로 초기화) |
...
| 분류 | 조건 | 전략 |
|------|------|------|
| **단순 실패** | 실패 테스트 ≤2개, 새로운 에러 | 즉시 수정 (같은 세션, worker 대상 fix) |
| **반복 실패** | 동일 에러 3회 연속 | 세션 초기화 (새 세션, "다른 접근법" 명시) |
| **구조적 실패** | 실패 테스트 >50% 또는 pass rate 10%+ 하락 | Circuit breaker (Phase 1 재계획) |
...
- **설계 문서**: [`docs/design_unified_agent_methodology.md`](design_unified_agent_methodology.md)

### 영향을 받은 프로젝트

UAM의 설계는 다음 에이전트 오케스트레이션 시스템의 아이디어를 참고했다:

- **[oh-my-claudecode (OMC)](https://github.com/nicobailey-llc/oh-my-claudecode)** — Claude Code용 멀티 에이전트 오케스트레이션 레이어. 병렬 실행, 키워드 자동활성화, 상태 대시보드.
- **[hoyeon](https://github.com/hoyeonkim795/hoyeon-claude-code)** — 엄격한 오케스트레이터-워커 분리, Fat Skill 아키텍처, 서킷 브레이커 패턴.

---

**UAM은 독립형 에이전트 오케스트레이션 플러그인이다. 외부 의존성 없이 Claude Code + Node.js만으로 동작한다.**
